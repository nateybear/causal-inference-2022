...
## Quick Hadley Appreciation Slide
- Hadley Wickham is "Chief Scientist" at RStudio
- Author of many [tidyverse](https://www.tidyverse.org/) core packages and its chief evangelist

![](rdemo_assets/hadley.jpeg)

- Two of my favorite Hadley talks: [here](https://www.youtube.com/watch?v=9YTNYT1maa4) and [here](https://www.youtube.com/watch?v=bzUmK0Y07ck)
...
```
- This function is not pure—we are changing the global variable y each time the function is called:
```{r}
y <- 0
impure_function <- function(x) {
  y <<- y + x
  y + 3
}
```

## FP Building Block: First-Class Functions
- A key part of functional languages is that functions can be assigned to variables, used as arguments to functions, etc.
...
- The pipe operator from `magrittr` is a key part of our FP toolkit
  ...
  - `x %>% f(y)` gets translated to `x %>% f(x,y)`
...
- As academics who produce reports, we can think of R scripts as pipelines that operate on our saved data files and produce <b>some</b> output: a figure, another dataset, an R markdown file...
  - In a sense, our R scripts should look like one long string of pipes! Keep this in mind throughout the rest of these slides.

## If you really want to go there...
- [So what's Haskell?](http://learnyouahaskell.com/introduction#so-whats-haskell)

<iframe width="400" height="275" src="https://www.youtube.com/embed/6mTbuzafcII" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
- A [cheatsheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) from RStudio
...
- This is a panel dataset, so we probably don't want to display data for all years
  - In grammar of graphics, time is another "dimension" we could map to an aesthetic feature
...
- References:
  ...
  - Cheetsheat on [data transformation](https://github.com/rstudio/cheatsheets/raw/master/data-transformation.pdf) 
...
- Advanced usage includes the `across` verb
  - See dplyr's [colwise vignette](https://dplyr.tidyverse.org/articles/colwise.html) for more info
```{r}
# make log columns, perhaps for modeling. The {.col} is called "glue syntax"
gapminder %>%
  mutate(across(lifeExp:gdpPercap, log, .names = "log_{.col}")) %>%
  head(5)
```
...
- Practical example: people do not agree on spellings of Afghan provinces
  - Compare SAP data to other indicator data (forgot where I got it)
```{r, cache=TRUE, warning = FALSE, message=FALSE}
sap_provinces <- 
  vroom(here("data/SAP_2019_clean.csv.gz")) %>% 
  distinct(province = m7)

other_provinces <- 
  readxl::read_xlsx(here("data/Afghanistan Database.xlsx")) %>% 
    clean_names %>% 
    distinct(province = name)
```
- Coding footnotes
  - `distinct` returns a one-column tibble of distinct values
    - The two tables have different names for the province column so, e.g. `province = m7`, is renaming `m7` from the SAP dataset to a better name, `province`.
  - `clean_names` changes column names to lower case and replaces spaces with underscores. Easier to program with data after cleaning names. If you want to go the "other" direction, check out the [`snakecase`](https://github.com/Tazinho/snakecase#snakecase-) package
  
## Use `anti_join` to inspect mismatches{.smaller}

- What provinces are in SAP but not the other dataset?

```{r}
anti_join(sap_provinces, other_provinces, "province") %>% arrange(province)
```

## {.smaller}

- What provinces are in the other dataset but not SAP?

```{r}
anti_join(other_provinces, sap_provinces, "province") %>% arrange(province)
```

## Solution: Translation Table {.smaller}
- Look at this on your own time
```{r, code = readLines(here("R/describe/RRRA_corruption_correlates.R"))[8:28], eval=F}
```

---

- Now, we can join this indicator data with the `corruption_data` table that I compiled from SAP
```{r, code = readLines(here("R/describe/RRRA_corruption_correlates.R"))[30:33], eval=F}
```
- N.B. joins are very similar `filter` if you think about it
  - Can be useful for filtering data based on values in a different table
    - Then just [`coalesce`](https://dplyr.tidyverse.org/reference/coalesce.html) any NAs if you use full/left/right joins
  - More info on joins [here](https://dplyr.tidyverse.org/articles/two-table.html)
